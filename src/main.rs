    ...
    RW: AsyncRead + AsyncWrite + Unpin + Send,
{
    ...
    macro_rules! reply {
        ($status:expr) => {{
            let _ = sock_w.write(&http_response($status, $status)).await;
            return;
        }};
        ($status:expr, $message:expr) => {{
            let _ = sock_w.write(&http_response($status, $message)).await;
            return;
        }};
    }
    ...
    loop {
        if let Err(e) = read.fill_buf().await {
            match e.kind() {
                std::io::ErrorKind::UnexpectedEof => {}
                _ => debug!("{remote}: read failed: {e}"),
            }
            return;
        }
        ...
        // done reading header, connect to the endpoint
        let content_length = reader.content_length;
        let mut read = reader.done();
...
async fn handle_http2_connection(
    ...
    let Ok(mut backend) = connect_tls(backend, endpoint, b"\x02h2").await else {
        return;
    };

    if let Err(e) = io::copy_bidirectional(&mut stream, &mut backend).await {
    ...
    B: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let copy_req = async move {
        let r = match req_content_length {
            None => {
                let r = tokio::io::copy_buf(&mut client_read, &mut backend_write).await;
                let _ = backend_write.shutdown().await;
                r
            }
            Some(l) => {
                let r = tokio::io::copy_buf(&mut client_read.take(l), &mut backend_write).await;
                if r.is_err() {
                    let _ = backend_write.shutdown().await;
                }
                r
    ...
    let copy_ok = match response_length {
        None => {
            let ok = {
                ...
                pin!(copy_resp);
                copy_req_and_resp(copy_req, copy_resp).await
            };
...
async fn copy_req_and_resp<Req, Resp>(mut copy_req: Req, mut copy_resp: Resp) -> bool
where
...
where
    ...
    W: AsyncWrite + Unpin,
{
    let cbpr = http1::CopyingBytePeekRead::new(input, output);
    let mut reader = http1::Reader::new(cbpr, None);
